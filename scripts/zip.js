...
const BUNDLES = {
  api: ['dist-api.zip', 'api', 'node_modules', 'package.json'],
  web: ['dist-web.zip', 'web', 'node_modules', 'package.json', 'Procfile'],
};

function build(which) {
  const [out, ...items] = BUNDLES[which] || [];
  if (!out) throw new Error(`unknown bundle "${which}" (expected: ${Object.keys(BUNDLES).join(', ')})`);
  fs.rmSync(out, { force: true });
  execFileSync(TAR, ['-a', '-c', '-f', out, ...items], { stdio: 'pipe' });
